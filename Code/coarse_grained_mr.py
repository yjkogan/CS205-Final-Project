...
class MySimTeachers(MRJob):
    ...
    def mapper(self, key, value):
        ...
        row = value.split(';')
        try:
            row = map((lambda x: ast.literal_eval(x)),row)
            num_teachers = len(row[0])
            teachers = zip(*row)
            #calculate the score
            for t in teachers:
                print t[0]
                score = get_score(self.teacher,t)
                #Add this score to the list of teachers.
                #If no meaningful comparisons happened, add 0
                try:
                    self.comparedts.append((int(t[0]),score[0]/score[1]))
                except ZeroDivisionError:
                    self.comparedts.append((int(t[0]),0))
                except ValueError:
                    pass
        except SyntaxError:
            pass
