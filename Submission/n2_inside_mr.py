...
class MySimTeachers(MRJob):
    ...
    def mapper(self, key, value):
        ...
        if 0:
            yield
        row = value.split(',')
	# Open the file from which we will read teachers
	# We compare the row we read in against ever teacher in this file
	# in order to fill an entire row of the matrix with one mapper.
	# This allows us to run everything without making multiple calls to
	# emr, saving the startup time.
        ...
        for r in self.reader:
           r[0] = int(r[0])
	   #Only compare if current read in teacher has a higher id
           if(row[0] < r[0]):
              i += 1
